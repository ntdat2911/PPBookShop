document.addEventListener('DOMContentLoaded', function () {
  ...
  function addSelectedItem(value, text) {
    if (!selectedItemsContainer.querySelector(`[data-id="${value}"]`)) {
  ...
  function addDropdownItem(value, text) {
    if (!dropdownItems.querySelector(`[data-value="${value}"]`)) {
      const item = document.createElement('div');
      item.className =
  ...
  // Prepopulate selected items on load
  ...
  initialSelectedItems.forEach((item) => {
    ...
    // Directly remove item from dropdown without calling addSelectedItem
    const dropdownItem = dropdownItems.querySelector(`[data-value="${value}"]`);
    if (dropdownItem) {
      dropdownItem.remove();
    }
